...
def validar_texto(valor: str, campo: str) -> str:
    ...
    if not re.match(patron, valor):
        raise ValueError(f" {campo} solo puede contener letras, números, espacios o guiones.")
    return valor.strip()
...
def ver_funciones(funciones):
    ...
    for atributo in ["id_funcion", "id_pelicula", "sala", "hora", "asientos_disponibles"]:
        tabla.add_column(atributo.capitalize(), justify="center", style="green")

    for fn in funciones:
        tabla.add_row(fn["id_funcion"], fn["id_pelicula"], fn["sala"], fn["hora"], fn["asientos_disponibles"])
...
def crear_funcion():
    ...
    try:
        ...
        asientos_disponibles = validar_entero(input("Asientos disponibles: "), "Asientos disponibles")
        ...
        if funcion["id_funcion"] == id_funcion:
            ...
            try:
                funcion["id_pelicula"] = validar_texto(
                    input(f"Nuevo ID de película (actual: {funcion['id_pelicula']}): ") or funcion["id_pelicula"],
                    "ID de película"
                )
                funcion["sala"] = validar_texto(
                    input(f"Nueva sala (actual: {funcion['sala']}): ") or funcion["sala"],
                    "Sala"
                )
                funcion["hora"] = validar_texto(
                    input(f"Nuevo horario (actual: {funcion['hora']}): ") or funcion["hora"],
                    "Hora"
                )
                funcion["asientos_disponibles"] = str(validar_entero(
                    input(f"Nuevos asientos disponibles (actual: {funcion['asientos_disponibles']}): ") or funcion["asientos_disponibles"],
